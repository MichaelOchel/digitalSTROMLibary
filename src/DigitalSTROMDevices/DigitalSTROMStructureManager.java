...
public class DigitalSTROMStructureManager {
	...
	private Map<Integer, Object[]> zoneGroupNameMap;
	
	
	public DigitalSTROMStructureManager(List<Device> referenceDeviceList){
		//this.deviceList = referenceDeviceList;
		zoneGroupDeviceMap = Collections.synchronizedMap(new HashMap<Integer, HashMap<Short, List<Device>>>());
		//putDevicesToHashMap(deviceList);
		handleStructure(referenceDeviceList);
	}
	public DigitalSTROMStructureManager() {
	...
	public Map<DSID, Device> getDeviceMap(){
		return this.deviceMap != null?this.deviceMap:new HashMap<DSID, Device>();
	}
	...
	public HashMap<Short, List<Device>> getGroupsFromZoneX(int zoneID){
		return this.zoneGroupDeviceMap.get(zoneID);
	}
	
	public List<Device> getReferenceDeviceListFromZoneXGroupX(int zoneID, short groupID){
		return this.zoneGroupDeviceMap.get(zoneID) == null? null:this.zoneGroupDeviceMap.get(zoneID).get(groupID);
	}
	...
	public Device getDeviceByDSID(DSID dSID){
		if(this.deviceMap != null){
			return this.deviceMap.get(dSID);
		}
		return null;
	}
