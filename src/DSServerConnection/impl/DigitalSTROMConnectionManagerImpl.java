...
import DSServerConnection.DigitalSTROMConnectionListener;
...
public class DigitalSTROMConnectionManagerImpl implements
	...
	private void onNotAuthentificated(){
		...
		if(configuration.get(APPLICATION_TOKEN) != null && 
			...
			if(digitalSTROMClient.checkConnection(sessionToken) == HttpURLConnection.HTTP_OK) {
				...
				if(connListener != null){
					connListener.connectionStateChange(DigitalSTROMConnectionListener.NOT_AUTHENTIFICATED, DigitalSTROMConnectionListener.WRONG_APP_TOKEN);
				}
			}
		} else{
		...
		if(checkUserPassword(configuration)){
			if(!isAutentificated){
				...
				if(applicationToken != null && !applicationToken.trim().isEmpty()){
					...
					if(this.digitalSTROMClient.enableApplicationToken(applicationToken, sessionToken)){
						...
						if(connListener != null){
							connListener.connectionStateChange(DigitalSTROMConnectionListener.NOT_AUTHENTIFICATED, DigitalSTROMConnectionListener.WRONG_USER_OR_PASSWORD);
						}
					}
				}
			}
			...
			if(!isAutentificated){
				...
				if(connListener != null){
					connListener.connectionStateChange(DigitalSTROMConnectionListener.NOT_AUTHENTIFICATED, DigitalSTROMConnectionListener.NO_USER_PASSWORD);
				}
			}
	}
	...
	private void onConnectionLost() {
        ...
        if(connListener != null){
        	connListener.connectionStateChange(DigitalSTROMConnectionListener.CONNECTION_LOST);
        }
        //in Connlistener 
        /*
	...
	private void onConnectionResumed() {
       ...
       if(connListener != null){
    	   connListener.connectionStateChange(DigitalSTROMConnectionListener.CONNECTION_RESUMED);
       }
    }
